...
def get_real_name(i: int, direct, natural):
...
def make_real_table(methods):
    ...
    for direct in [False, True]:
        for natural in [False, True]:
            ...
            for i in range(5):
                name1 = get_real_name(i=i, direct=direct, natural=natural)
...
def lineplot(type):
    ...
    plt.savefig('test-data/' + type + 'lineplot.png', dpi=600, bbox_inches='tight')
    # plt.savefig('test-data/' + type + 'lineplot.eps', format='eps', bbox_inches='tight')
    plt.show()
...
if __name__ == '__main__':
    ...
    ind_size = 5
    make_real_table(methods=methods)
