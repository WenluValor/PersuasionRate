...
from realvarest import get_var
...
def boots_bound(rep_B: int, sam_B: int, po_num, F, direct, natural):
    ...
    RR_res = np.zeros([rep_B])
    v_RR_res = np.zeros([rep_B])
...
def est_bound(po_num, direct, natural):
    ...
    cov_F = np.array(pd.read_csv('real-data/var_F.csv', index_col=0))
    inf, sup, RR, v_inf, v_sup, v_RR \
...
def ora_bound(po_num, case, n_val, direct, natural):
    ...
    for y_val in range(2):
        if CASE == 'hmm':
            U = np.array(pd.read_csv('real-data/U.csv', index_col=0))[0: (N + 1)]
