...
def set_global(n_val):
    ...
    Y = np.array(pd.read_csv('test-data/Y.csv', index_col=0))[0: (N + 1)]
...
def est_pi():
    ...
    pkl_filename = 'test-data/pi_model.pkl'
...
def get_fpi_res():
    ...
    f_filename = 'test-data/f_model.pkl'
    with open(f_filename, 'rb') as file:
        f_model = pickle.load(file)
    pi_filename = 'test-data/pi_model.pkl'
...
def compute_F(f_res, pi_res, IPW=False):
    ...
    F_res = F[N]
    var_F = compute_A(F=tmp_F)
    return F_res, var_F


def compute_A(F):
    """
    F: numpy array of shape (N+1, 4)
    Uses psi_k = F[k] for k = 1,...,N
    """
    # Step 1: extract psi (ignore F[0])
    psi = F[1:]            # shape (N, 4)
    psi_bar = psi - psi.mean(axis=0, keepdims=True)   # shape (N, 4)
    A = (psi_bar.T @ psi_bar) / N
    # Step 4: second term (autocovariance part)
    L = int(N ** 0.25)
    for k in range(1, L):
        A_1 = psi_bar[:N-k]      # shape (N-k, 4)
        A_2 = psi_bar[k:]        # shape (N-k, 4)
        Gamma_k = (A_1.T @ A_2) * (1 - k / (L + 1))   # shape (4, 4)
        A += 2 / N * Gamma_k
    return A
