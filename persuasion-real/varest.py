...
def est_Di(po_num, F, natural):
    ...
    for i in range(1, N + 1):
        if i == 1:
            res[i] = ((2 * T[i]) == np.arange(4)).astype(int)
        else:
...
def est_de_num(po_num, F, natural, direct):
    # P(Y(1) = 1| Y(0) = 1), return derivative D0 = P(Y(0) = 1), D1 = P(Y(1) = 1)
    N = po_num.shape[0] - 1
...
def get_deriv(type, po_num, F, natural, direct):
    N = po_num.shape[0] - 1
    # D1 is derivative of numerator; D0 is derivative of denominator
    ...
    if type == 'inf':
        for j in range(4):
            tmp[:, j] = np.where(denom != 0, 1 / tmp_denom * np.where(D1[:, j] > D0[:, j], D1[:, j] - D0[:, j], 0)
